...
while Number_Of_Pictures < 5000:
        ...
        Send_String= str(datetime.now().strftime("%d/%m/%Y %H:%M:%S")) + ',' + str(round(datetime.now().timestamp()-Initial_T.timestamp(),2)) + ',' + Send_String 
        print(Send_String)
        
        with open("/home/pi/Documents/Python/Pi_Mangeoire_Python/Data_Ina219.txt","a")  as Data:
             Data.write(Send_String)

        if Solar_Voltage < 12.8 :# or Solar_Current > 0 :
...
#sys.exit()
#cam.stop()
print("Limite du nombre d'images atteinte")            
